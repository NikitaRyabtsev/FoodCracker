...
//Meals
	String SQL_QUERY_CREATE_MEAL = "INSERT INTO Meal (date,time,weight,calories,user_idUser) VALUES (?,?,?,?,?)";
	String SQL_QUERY_GET_MEAL = "SELECT * FROM Meal WHERE idMeal=?";
	String SQL_QUERY_GET_DATE = "SELECT * FROM Meal WHERE date=?";
	String SQL_QUERY_GET_ALL_MEAL = "SELECT * FROM Meal WHERE user_idUser=?";
...
//Food
	String SQL_QUERY_GET_ALL_FOOD = "SELECT food.* FROM meal JOIN food_has_meal ON meal.idMeal = food_has_meal.meal_idMeal " +
			" JOIN Food ON food_has_meal.food_IdFood = food.IdFood " +
			" WHERE meal.idMeal = ? AND user_idUser = ?";
	String SQL_QUERY_CREATE_FOOD = "INSERT INTO Food(name,calories,proteins,fats,carbohydrates) VALUES(?,?,?,?,?)";
}
